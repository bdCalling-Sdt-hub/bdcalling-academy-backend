...
use App\Http\Requests\PhoenixBatchRequest;
use App\Models\Batch;
use App\Models\BatchTeacher;
use App\Models\Course;
use App\Models\PhoenixBatch;
use App\Models\Teacher;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;

class PhoenixBatchController extends Controller
{
    public function index(Request $request)
    {
//        $query = Batch::with('teachers.user');
        $query = Batch::with('course')->where('batch_id','like','%'. 'Phoenix'. '%');
    ...
    public function store(PhoenixBatchRequest $request)
    {
        DB::beginTransaction();
        try {
    ...
    private function createBatch($course_id, $batch_type)
    {
        try {
            ...
            $course_count = PhoenixBatch::where('course_id', $course_id)->count();
            $count_formatted = str_pad($course_count + 1, 2, '0', STR_PAD_LEFT);
            $academy_name = 'Phoenix';
    ...
    {
        try {
            $batch = Batch::with('course','teachers')->where('batch_id', 'like','Phoenix%')->where('id',$id)->first();
            if (!$batch) {
                return response()->json([
